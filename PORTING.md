...
| C35 | **`edm vendor` exists.** Bun answers `Unknown command "vendor"` and exits 2. | A third command in the disjoint `EXTENDED_COMMANDS` set. It implements the observed read-only `GET /2.0/elite/vendors/items` envelope (`cmdrId`, station-scoped `marketId`, `vendorType=1`, credentials), uses Ardent only to resolve system/station names into those market IDs, and aggregates Pioneer Supplies stock without changing the four oracle-pinned commands. |
