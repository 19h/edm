...
| C52 | **`edm sell --follow <s>` re-plans the disposal on an interval instead of printing once**: each round re-reads the journal for the hold and the ship's position, re-reads every nominated buyer live, and solves again from what is left. It stops when the hold is empty. | The request is C43's for the other command, and the shape is different in one way that matters: a route ranking is a list to choose from, but a sell plan is a sequence to *execute*, and executing it changes the input. After the first stop the hold is smaller and the ship is somewhere else, so the plan printed at departure is wrong the moment it is acted on — and the second stop was chosen against a hold that no longer exists. A round is therefore the last three phases again rather than route's verify pass: `manifest` over a quiet journal re-read \[C49\], one sweep of the nominated buyers through the refresh-mode cache, and `solve`, which is now the one door the initial run, the verify pass and every round go through so a plan is always built the same way from whatever was read last. **Every nominated buyer is re-read, not only the plan's.** Route re-reads the markets behind its ranked routes because its candidate set is thousands and the ranking is the point; sell's candidate set is already `--top` per commodity aboard, the gate priced exactly that many reads, and re-reading only the chosen stops would mean a plan that could never move to a better buyer once its own filled up — which after a sale is the normal case, not the exception. **The buyer set is fixed, so cargo taken aboard since the search is named and not planned.** A round never asks Ardent again; that is what makes it a sweep. A stack the first nomination never saw has no buyers here and would silently fall off the plan, reading as "nobody buys this" — so it is said once, with "re-run to plan it". **`--from` pins the origin; without it the origin follows the ship**, because the flag's own help says it is for when the journal does not say, and a plan that starts from where the ship was is a number about the past. **The bar is re-derived every round** from the hold now aboard and the prices just read, as the initial run derives it; only the verify pass carries the seed bar forward, since it is the commander's standard for a further stop rather than a fact about a buyer. The stop conditions are C43's and C46's — `--follow-rounds`, `--max-requests` enforced live, three consecutive rounds with nothing to plan — plus the one this command has and route does not: an empty hold, which is the loop having done its job. Refused with `--json` for C28's reason. The test harness gained a paused-clock driver so a thirty-second loop is exercised end to end in milliseconds, with the wire trace unchanged. |
| C53 | **`edm ui` is a full-screen front end over the same route, survey and sell pipelines**: a search form with completion, the ranking as a table with a live column, route pinning, and a detail screen that keeps one pinned route fresh — its markets re-read live, carrier doors checked, position tracked — on its own interval. | The commands print once and `--follow` re-prints whole tables; a commander flying a route wants the opposite shape, one route kept fresh and the rest on demand. **The pipeline is the same code, reached through three seams rather than a copy.** `Out` gained a forwarding sink so every word a search says lands in a log pane instead of a raw-mode terminal; `plan::decide` split the gate's arithmetic and table from its printing, and a `Gate` hook lets the console print its instruction and decline while the screen asks in a modal with the same plan; `route::run`, `quick::run` and `sell::run` were each split into a `search` stage that returns the solved instance and a `round` stage the follow loops and the screen both call, with a `Solver` hook so the survey's solve — minutes at region scale — runs on a thread while the screen keeps drawing. **Every search inside the UI is an argv**, parsed by the extended table and read by the same config readers, so validation and defaults are the commands' own; the goldens and the parity harness are what prove no byte of any command's output moved during the extraction. **One pacer per session**, opened with `begin_round` where each search's first paid request can happen, so `--max-requests` is a session ceiling enforced live and two jobs cannot double the rate \[C37\]; the screen runs one network job at a time for the same reason, with free Ardent lookups for completion beside it. **A pin is re-priced, never re-searched:** its markets are read live through the refresh-mode cache, the route is rebuilt as a skeleton from a `PinKey` (market ids and commodity names in `RankKey`'s canonical rotation, stable across solves and interners) and handed to `rescore`, which prices it with its commodity fixed or drops it, so a pin can never quietly become a different route; carrier doors go through `access::prepare`/`probe`/`finish` with the journal overlaid \[C42\]. Pins and the last search are JSON under the cache root, written by this program's own serializer. **Signals, per R96:** nothing here traps SIGINT — raw mode makes Ctrl-C a key the loop treats as quit, and the terminal is restored by a guard's `Drop` and a panic hook that runs before the default one; crossterm's SIGWINCH handler (signal-hook, not tokio's `signal` feature, which stays off) is the one signal the binary listens to, and only while this command runs. Terminals that report key releases would otherwise type every character twice, so only presses are read; found live under a pty, where the search ran for "SolSol". |
| C54 | **The journal parser keeps a bounded history of visited systems and docked stations**, newest first, surviving `LoadGame`. | Completion needs names a commander is likely to type, and the journal is the one source that knows where they have actually been; `CommanderState` held only the current system. Sixty-four each, deduplicated, because this is a completion list rather than a log. Not cleared on `LoadGame`, for the reason `carrier_doors` is not: a name is no less familiar for a restart. Serialised with `#[serde(default)]` so an older state still reads. |
...
| C44 | **The QUICK LOOKUP table gains `Ly`: how far each nominated seller or buyer is from the reference system.** The market id is demoted below it in the drop order. | The table's whole job is to say which candidates are worth reading live, and it did not carry the one number that decides that. A commander reading `sells 4,520, 13,953 t` has no way to tell a station next door from one ninety light years out, and the two are not the same offer at the same price — reported from a live run where the cheapest seller of Micro Controllers was 91.5 Ly away and the second-cheapest, at the same 4,520 a ton, was 64.4. The sibling BEST LIVE PRICES table has carried a distance column since C40; this one was simply missed, so the pre-verification view was less informative than the post-verification one about the decision that happens *before* verification is paid for. The id is demoted rather than dropped because the TRADE COMMANDS block already prints it for every route, so its only remaining use in the table is copy-paste — which loses to knowing whether the station is reachable this session. Measured from the same reference the search is centred on, `Geometry`-free via `time::distance_ly`, so it costs one square root per row and no requests. |
| C45 | **An extension command's argv error prints that command's help, not the ported one.** `Parsed` gains `misread_command`, and `cli::usage_for` maps a command to its text. | The complaint already came from the extended table -- C26 exists precisely because the base table cannot see route-only names, so `edm route --quick --radius 200` had to stop reporting `Unknown option --quick`. But the *help* printed underneath it was still `usage()`, the ported Bun text, which documents `market`, `trade` and `markets` and has never mentioned a route flag in its life. So the pair contradicted each other: the message named a flag and the help beneath it proved that flag did not exist. Reported from a live run of `--follow` with no value, where the reader reasonably concluded the option had not been implemented -- it had, and `edm route --help` documented it four times. This is the same argument C26 makes about the message, applied to the text that explains it, and the fix is keyed the same way: on argv's first token, so a ported command's argv can never reach it and R49's byte-pinned usage is untouched. Verified both directions: `route --follow` now prints route's help, `trade --qty` still prints Bun's. |
| C46 | **A `--follow` session stops after three consecutive rounds in which nothing in the shortlist prices**, and the ingest counters are printed once rather than under every round. | `--follow` re-prices a fixed candidate set and never re-nominates, which is what makes a round cheap — but it means the loop cannot recover from the set dying. Observed live: a twenty-route ranking whose every route ended at the same fleet-carrier buyer, `T9K-1VL` in Eurybia, holding a 556 t tritium order. At round 62 the order was filled or withdrawn — the row vanished from the carrier's market entirely rather than falling to zero demand — and all twenty routes lost their sell side in the same instant. Rounds 62 onward then re-read the same 21 markets every 30 s to print `no profitable hop in this data`, and would have gone on doing so until the request ceiling, because no round can nominate the buyer that replaced it. **Three rounds, not one:** a single empty round is a transient read failure, since a market that timed out drops its route for that round alone; three consecutive is the candidate set genuinely gone. The concentration is the underlying hazard and it is not a bug — the ranking is *supposed* to converge on the best buyer, and when one carrier outbids every station within the radius, every route legitimately ends there. What that produces is a shortlist with a single point of failure, so the collapse is total rather than gradual, and the message says which of the two happened. Separately, `Crossing`'s counters (`2,127 commodity rows carried a non-integral price...`) describe the first ingest and are never recomputed by a re-read, so restating them under all sixty-eight rounds asserted a measurement no round had made. |
| C47 | **`--by-profit` orders routes by credits per run and ignores travel time entirely.** The objective lives on `Geometry`, is stamped into every `RankKey`, and `Ord for RankKey` branches on it. | The default answers "what should I fly", and for that a rate is right: a 118 Mcr hop three hundred light years out really is worse than a 60 Mcr hop next door if you are going to fly it repeatedly. But it is the wrong question for a hauler already heading that way, or a hold that has to be filled exactly once, and the table gave no way to ask the other one — a reader wanting the biggest single payout had to eyeball the profit column against an order that was not sorted by it, which is the same complaint C39 fixed for the missing rate column, in the opposite direction. **The objective belongs on the key, not on the display sort.** The search heaps in `single` and `round` keep their best N *by `RankKey`*, so a high-profit, low-rate route is evicted mid-search and can never reach a sort that wanted it — ordering the finished list would silently rank the top-N-by-rate rather than the top-N-by-profit, which is the truncation trap `budget_survives_ranking` exists to catch. **It rides on `Geometry` because `Geometry` is `Copy` and already reaches `Route::single_hop` and `Route::cycle`**, the two places a key is built; the alternative was widening both public signatures and every call of them. Both orderings end in the same absolute station/commodity tie-break, so a shuffled input still produces an identical ranking, and `Profit` still breaks an exact tie toward the shorter flight — between two identical payouts, less flying is strictly better. Comparing keys built under different objectives is meaningless and cannot arise: the objective comes from the geometry and one search has one geometry. The ordering note reads the objective off the routes rather than taking a parameter, for the same reason the key carries it. `Ly` and `To start` stay on screen and stop deciding, which the note says outright. |
| C48 | **`--from-here` pins every route's origin to the market the commander is docked at**, or to their current system's markets when undocked. | The region-wide ranking cannot answer "what should I load *right now*", and not by oversight: `--quick` nominates from Ardent's price index, which pages the top thousand rows per commodity **by price**, so the berth under the ship is almost never in it. Post-filtering a finished ranking is therefore useless twice over -- the origin's rows were never nominated, and even if they were, the search would rank every seller in the region, truncate to `top_n`, and only then discard the ones that did not start here, so a route from this station would be dropped before it was ever seen whenever twenty better ones existed elsewhere, which is the normal case. **The pin is applied by clearing `Market::supply` everywhere else**, between ingest and solve. With no supply anywhere but here, every hop the solver can build already departs from here, `top_n` counts routes from here rather than routes that survived, and no downstream stage needs to know the flag exists. Two supporting pieces: the departure station is **merged into the polled set**, because it has to be *read* and not merely permitted -- clearing every other market's supply when the origin was never nominated leaves the search with no seller at all and returns a confident empty answer; and the pin is **named in a note**, because "no profitable hop in this data" means something entirely different depending on whether nothing here is worth hauling or the program could not work out where here is. Verified live from `Tall Town`: `--category metals` correctly returns nothing (it stocks none) while the full run returns nineteen hops, all departing from it. Docked identity comes from `CommanderState::current_market_id`, which reads the journal's `Docked` event, so it costs no request; the undocked fallback uses Ardent's free system-markets page. |
| C49 | **A `--follow` round re-reads the journal**: the approach column tracks the ship as it flies, and with `--from-here` the loop stops the moment the ship leaves the station its shortlist departs from. | `--follow` held the origin it was given at startup, so two things went stale as soon as the commander moved. `To start` kept reporting the distance from wherever the run began, which is a number about the past presented as a decision aid — and it is the column C40 added precisely so a reader could judge reachability. Worse, under `--from-here` every route in the list departs from one specific berth \[C48\]; once the ship leaves it, those routes still *price* perfectly well and would keep being printed and re-verified indefinitely, so the failure is silent rather than empty. Stopping is the honest answer because the loop cannot fix it: re-pinning would need a fresh nomination and solve for the new origin, which is a full run, not a round. The journal is a local file, so noticing costs no request and the check is free every round. `reload_commander_state` exists rather than reusing `local_commander_state` because that one warns about malformed observations — true and worth saying once, noise sixty-eight times, and it would bury the ranking it sits above. |
| C50 | **The sell alternatives table names each stop's system and total flight, both plans' trade commands name the system, and a duration never reads `60s`.** | Three defects in one block, all found by a commander who could not act on what it told them. **`H7H-75X` names nothing findable:** it is a fleet-carrier callsign, and the row offered it as a plan worth thirty million more without saying which system it is in — C44 fixed exactly this for the quick-lookup table and the reasoning was never carried across. **The command list was the worse half and was missed on the first attempt.** Putting the system in the table alone is not enough: a table cell is width-bound and truncates (`CAZU (Col 285 Sector CC-K ~`), while the command list underneath is the thing a commander copies, and it printed a bare `at H7H-75X` with no system at all. The route command block has always named systems in its per-route header; sell's simply never did. Labels are now measured across every line of both plans so the commands stay in one column rather than being padded to a constant the longest name overruns. **An alternative with no commands is decoration:** the block listed only the recommended plan, so acting on the row it had just recommended against meant finding two market ids by hand — the work the block exists to save. **And `2m 60s` is not a time.** `duration_estimate` floored the minutes from the raw seconds while rounding the remainder independently, so a remainder of 59.5 or more carried to 60 without the minute noticing: 179.6 s rendered as `2m 60s`. Rounding to whole seconds before splitting fixes it at every boundary; the sub-90-second branch still ceils, unchanged. That one was live in every route lap, every sell plan and every spend estimate the program has ever printed. |
| C51 | **`--radius` may now reach 1,000 Ly, covered by tiling several 500 Ly queries.** Ardent's own clamp stays 500; `MAX_RADIUS_LY` no longer aliases it. | Measured, not assumed: `maxDistance=1000` and `maxDistance=2000` from Sol return exactly the rows `maxDistance=500` does, furthest at 494 Ly, `beyond500=0`. The server clamps and says nothing, which is why the ceiling was pinned to it. Covering more therefore means more queries, and the geometry has one hard constraint: **the endpoint is addressed by system name and there is no coordinate lookup** — `/system/position`, `/system/xyz`, `/system/near` and `minDistance`/`offset` are all 404 or silently ignored — so a tile centre must be a real system, and the only source of names out at the shell is the rows a previous query returned. Hence generations: query the reference, harvest every system its rows name, pick the next ring from those, repeat. Ring selection is **farthest-point sampling** over candidates in the outer band (0.6·reach to reach) — take the one furthest from the reference, then repeatedly the one furthest from everything already chosen — because picking by distance alone clusters the whole ring in whichever quadrant happens to hold the most markets, leaving the rest of the shell unqueried. Capped at 25 centres and 12 per generation, since each centre costs three free Ardent pages *per commodity* and a wide radius over a category is already thousands of requests against somebody else's server. Rows outside the requested ball are dropped after merging: a tile is a ball around somewhere else and reaches past the radius that was asked for, and `--radius` is a promise about the answer rather than about how it was gathered. Markets inside two tiles are deduplicated by market id. **The result is a union of prefixes, not a prefix**, and both the table note and the help say so: each tile carries its own 1,000-row cap, so the outer shell is sampled rather than surveyed and a market there can be missed entirely. Verified live at 1,000 Ly: 9 candidate rows beyond 500, furthest 579.8 — unreachable from any single query. |
