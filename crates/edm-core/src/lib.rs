...
//! `cargo xtask gates` fails the build if an impure dependency ever appears
...
//! not share — `f64`-only arithmetic, UTF-16 string indexing, ECMAScript
//! object key enumeration order, and `Number::toString`. Rather than dust
//! those conversions across the codebase, they are concentrated in [`js`],
//! which everything else is written in terms of.
