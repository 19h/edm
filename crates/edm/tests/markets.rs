...
fn markets_end_to_end() {
    ...
    run.assert_exit(0);
    assert!(
        run.stdout.contains("(round-trips)"),
        "Colonia's address must re-pack"
    );
    ...
    assert!(
        run.stdout
            .contains("1 without a commodity market hidden by --trading")
    );
    ...
    // dumped \[C16\].
    let run = drive(
        &["markets", "Colonia", "--dump", "/tmp/starsystem.json"],
        &http(),
    );
    run.assert_exit(0);
    ...
    assert!(
        run.stdout
            .contains("of starsystem payload to /tmp/starsystem.json")
    );
    ...
    let odd = FakeHttp::default().route(
        ...
        vec![sealed(
            r#"{"stations":[{"name":"Somewhere","marketId":42,"type":"Outpost"}]}"#,
        )],
    );
    ...
    assert!(
        run.stdout
            .contains("== POINTS OF INTEREST  1 found by scan ")
    );
    ...
    assert!(
        run.stdout
            .contains("no markets found under starsystem.polities")
    );
