...
pub async fn run<H: HttpTransport, C: Clock, E: Entropy, F: Fs>(
    ...
    if let Some(resolved) = &resolved {
        ...
        rows.push(field(
            ...
            if inside {
                ...
                "NO \u{2014} address and coordinates disagree".to_owned()
            },
        ));
        rows.push(field(
            "repacked address",
    ...
    let query = config::starsystem_query(&app.cli, CachedTimestamp::Flag).map_err(message)?;
    let stamp = app.stamp()?;
        ...
        app.emit_json(
            ...
            vec![
                (
                    ...
                    resolved
                        .as_ref()
                        .map_or(JsValue::Null, super::market::resolved_json),
                ),
    ...
    if let Some(path) = app.cli.optional_value(Flag::Dump, None) {
        ...
        app.ports
            .fs
            .write(std::path::Path::new(path), text)
    ...
    if all.is_empty() {
        ...
        // ts:3081
        app.note(
            "starsystem.polities held no markets — falling back to a structural scan".to_owned(),
        );
    ...
    let trading_only = app
        .cli
        .switch_value(Flag::Trading, false)
        .map_err(message)?;

    let hidden_carriers = if include_carriers {
        0
    } else {
    ...
    let survivors: Vec<&MarketPoint<'_>> = all
        ...
        .collect();
    let hidden_idle = if trading_only {
    ...
    if hidden_carriers > 0 {
        skipped.push(format!(
            "{hidden_carriers} fleet carriers hidden (--carriers to show)"
        ));
    }
    if hidden_idle > 0 {
        skipped.push(format!(
            "{hidden_idle} without a commodity market hidden by --trading"
        ));
    }
    if !skipped.is_empty() {
    ...
    let target = station_of(resolved.as_ref()).and_then(|station| {
        ...
        points
            .iter()
            .find(|point| point.name.to_lowercase() == wanted)
    });
...
fn parts_json(parts: &AddressParts) -> JsValue {
    object([
        (
            "massCode".to_owned(),
            JsValue::Num(f64::from(parts.mass_code)),
        ),
        (
            "massCodeLetter".to_owned(),
            str_value(&parts.mass_code_letter.to_string()),
        ),
