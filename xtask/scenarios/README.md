...
| `expect-min-gap-ms` | | every game-internal API arrival must be at least this far after the previous one. Fewer than two arrivals is a failure, not a pass. |
| `expect-frontier-requests` | | exactly this many requests must reach the game-internal API. |
...
`route` does not exist in `game-internal-api.ts` (C25), so there is nothing to
...
encode = "game_api"        # raw | game_api | game-api-unframed | game-api-corrupt-block
